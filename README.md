...
**접속만** 하면 설치하지 않습니다. 브라우저에서 허브에 로그인하면 됩니다. 핸드폰도 웹만 씁니다.

원격으로 **열릴** Windows에는 호스트가 필요합니다. 브라우저만으로는 화면 캡처·로그인 화면·서비스를 할 수 없습니다. Chrome 원격 데스크톱과 같습니다.

웹에서 **설치 파일 받기**를 누르거나, 저장소에서 설치 프로그램을 만듭니다.

```bat
npm run pack:setup
```

`dist\RemoteAI-Setup.exe` 가 생깁니다. 그 PC에서 실행하면 프로그램 추가/제거에 등록됩니다. 설치 마법사에서 「로그인·잠금 화면에서도 대기」를 켜면 Windows 서비스로 올라갑니다 (관리자).
